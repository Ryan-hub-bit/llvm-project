...
static bool isIndirectCall(const CGCallee &Callee) {
  if (!Callee.isOrdinary()) {
    return false; // Not a normal function call (could be built-in, virtual,
                  // etc.)
  }

  llvm::Value *FuncPtr = Callee.getFunctionPointer();
  return !llvm::isa<llvm::Function>(
      FuncPtr); // Indirect if not a direct function
}
...
RValue CodeGenFunction::EmitCall(const CGFunctionInfo &CallInfo,
  ...
  if (isIndirectCall(Callee)) {
    ...
    if (TargetDecl && TargetDecl->getFunctionType())
      // Second argument (0): Specifies no additional qualifiers (e.g., not
      // const or volatile).
